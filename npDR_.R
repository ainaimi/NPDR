...
sl.lib_mu <- c(ranger_learner$names,svm_learner$names)
sl.lib_pi <- c(ranger_learner$names,svm_learner$names)
...
npDR<-function(counter,pNum,bs=T,bootNum=100){
  ...
  # plotDat <- tibble(pihatNPT,pi)
  # ggplot(plotDat) + geom_point(aes(pihatNPT,1-pi))
  ...
  if (bs==T){
    ...
    plugin2<-function(d,j,dim,f){
      ...
      mumod<-SuperLearner(Y,X,family=gaussian,SL.library=sl.lib_mu,cvControl=list(V=f))
  ...
  if(i==1&samp==1){
    ...
    write.table(ps.sl.coefNPT,"./results_sl-pi_true.txt",sep="\t",row.names=F)
    write.table(mu.sl.coefNPT,"./results_sl-mu_true.txt",sep="\t",row.names=F)
    write.table(ps.sl.coefNPF,"./results_sl-pi_misspec.txt",sep="\t",row.names=F)
    write.table(mu.sl.coefNPF,"./results_sl-mu_misspec.txt",sep="\t",row.names=F)
  } else{
    ...
    write.table(ps.sl.coefNPT,"./results_sl-pi_true.txt",sep="\t",row.names=F,col.names=F,append=T)
    write.table(mu.sl.coefNPT,"./results_sl-mu_true.txt",sep="\t",row.names=F,col.names=F,append=T)
    write.table(ps.sl.coefNPF,"./results_sl-pi_misspec.txt",sep="\t",row.names=F,col.names=F,append=T)
    write.table(mu.sl.coefNPF,"./results_sl-mu_misspec.txt",sep="\t",row.names=F,col.names=F,append=T)
  }
  return(tmp)
}
